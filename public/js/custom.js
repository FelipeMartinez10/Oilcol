...
function charts() {
	...
	if($("#stats-chart").length)
	{
		...
		// $("#temperatura").bind("plotclick", function (event, pos, item) {
		// 	if (item) {
	...
	/* ---------- Chart with temp ---------- */
	// LM Modificado para crear stadisticas de temperatura
	if($("#temperatura").length)
	{

		var temperatura = [];
		var cadena =document.getElementById("infoTemperatura").innerHTML.split(",");
		...
		for (var i = 0; i < x; i += 1) {
			...
			var temp = cadena[i];
			temperatura.push([i/2, temp]);
		}

		var plot = $.plot($("#temperatura"),
...
// $("#caudal").bind("plotclick", function (event, pos, item) {
// 			if (item) {
// 				$("#clickdata").text("You clicked point " + item.dataIndex + " in " + item.series.label + ".");
// 				plot.highlight(item.series, item.datapoint);
// 			}
// 		});
	...
	/* ---------- Chart with caudal ---------- */
	// LM Modificado para crear stadisticas de caudal
	if($("#caudal").length)
	{
		var caudal = [];
		...
		var previousPoint = null;
		$("#caudal").bind("plothover", function (event, pos, item) {
			$("#x").text(pos.x.toFixed(2));
			$("#y").text(pos.y.toFixed(2));
	...
	/* ---------- Chart with consumo ---------- */
	// LM Modificado para crear stadisticas de consumo
	if($("#consumo").length)
	{
		var consumo = [];
		...
		var previousPoint = null;
		$("#consumo").bind("plothover", function (event, pos, item) {
			$("#x").text(pos.x.toFixed(2));
			$("#y").text(pos.y.toFixed(2));
		...
		$("#consumo").bind("plotclick", function (event, pos, item) {
			if (item) {
				$("#clickdata").text("You clicked point " + item.dataIndex + " in " + item.series.label + ".");
				plot.highlight(item.series, item.datapoint);
			}
